...
<template>
    <main class="w-full h-full">
        <section class="px-[80px] py-[80px] w-full h-full flex justify-between">
            <section
                ...
                <section class="grid grid-cols-3 gap-6 gap-y-8">
                    <PizzaCard v-for="pizza in pizzas" :id="pizza.id" :name="pizza.name" :image="pizza.image"
                        :prices="pizza.prices" :isOpen='isOpen' @customize="pizzaToppingSearch(pizza.id)"
            ...
            <section
                ...
                <section class="flex flex-col gap-3">
                    <PizzaCart v-for="cart in carts" :cart='cart' :variant="variantSearch(cart.pizza_id)"
                        :toppings="toppingSearch(cart.topping_ids)" @delete-cart="deleteCart(cart.id, cart.totalPrice)"
                        @search="pizzaToppingSearch(cart.pizza_id)"
                        @update-cart="pushTopping"
                        @update-overall-prices="updateOverall(lastPrices)" />
        ...
        <AddVariant :isOpen='isOpen' @change-open="isOpen = !isOpen" @update-overall-prices="updateOverall(lastPrices)"
            :toppings="pizzaToppings" :pizzaId="pizzaId" :pizzaPrices="pizzaPrices" :updates="updates" :cart_id="cart_id" :toppingLists="toppingList" />
    </main>
</template>
